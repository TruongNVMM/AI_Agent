...
def _classify_text_block(text: str) -> BlockType:
    ...
    # Kiểm tra thêm LaTeX-like patterns như \frac, \sum_, ...
...
def _sort_reading_order(
    ...
    for b in raw_blocks:
        x0, y0, x1, y1 = b["bbox"]
        span_width = x1 - x0
        # Block trải dài > 70% chiều rộng trang → full-width (ví dụ: tiêu đề section)
    ...
    while l_idx < len(left_col) or r_idx < len(right_col) or f_idx < len(full_width):
        # Tìm y0 nhỏ nhất còn lại trong full_width
        ...
        if next_full_y <= next_left_y:
            ...
            # Đọc hết cột trái trước khi chuyển sang phải
...
def _crop_block_to_png(page: fitz.Page, bbox: tuple, dpi: int = RENDER_DPI) -> bytes | None:
    ...
    try:
        ...
        if pix.width * pix.height < OCR_MIN_CROP_AREA:
            log.debug("Bỏ qua crop quá nhỏ: %dx%d", pix.width, pix.height)
            return None
...
def segment_page(page: fitz.Page, page_num: int, layout: str) -> list[DocumentBlock]:
    ...
    Quy trình:
      ...
      3. Phát hiện tất cả hình ảnh → BlockType.IMAGE
    ...
    for b in page.get_text("blocks"):
        ...
        if b[6] != 0:
            continue  # Bỏ qua image block ở đây (xử lý ở bước 3)
        ...
        # Kiểm tra overlap với bảng đã phát hiện
    ...
    for idx, item in enumerate(ordered):
        ...
        if btype in (BlockType.IMAGE, BlockType.MATH):
            ...
            if crop_png is None:
                # Không crop được → fallback sang text nếu có content
                btype   = BlockType.TEXT
                content = content or "[Hình ảnh không thể trích xuất]"
    ...
    log.debug(
        "Trang %d: %d blocks (%d text, %d table, %d image, %d math)",
        page_num,
        len(document_blocks),
